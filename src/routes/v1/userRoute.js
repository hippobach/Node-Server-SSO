...
Router.route('/private/hook/login').post(
  authMiddleware.auth0JwtCheck,
  userController.hookLogin
);

// Private: API get toàn bộ user (cần xác thực accessToken)
Router.route('/private/get_all').get(
  authMiddleware.auth0JwtCheck,
  userController.getAll
);

// Public: API get toàn bộ user (Không cần xác thực accessToken)
Router.route('/public/get_all').get(userController.getAll);

// Public: API xóa một user theo email (dùng để test xóa user cho nhanh)
Router.route('/public/delete/:email').delete(userController.deleteByEmail);
